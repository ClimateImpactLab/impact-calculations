import re
import numpy as np
from adaptation.adapting_curve import InstantAdaptingStepCurve, ComatoseInstantAdaptingStepCurve, DumbInstantAdaptingStepCurve
from adaptation import curvegen, covariates
...
if do_singlebin:
    predcols = ['DayNumber-', 'loggdppc', 'logpopop']
else:
...
def prepare_interp_raw(csvv, weatherbundle, economicmodel, pvals, farmer='full', ageshare=False):
    if ageshare:
        ...
        assert csvv['L'] == 66
        for ll in range(66):
            assert re.match(prednames_ageshare[ll], csvv['prednames'][ll]) is not None, "{0} ~!= {1}".format(prednames_ageshare[ll], csvv['prednames'][ll])
