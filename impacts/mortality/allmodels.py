            ...
            for ageii in range(len(agegroups)):
                ...
                if config['do_farmers'] and not weatherbundle.is_historical():
                    ...
                    if check_doit(targetdir, subbasename + "-noadapt", suffix):
                        calculation, dependencies, baseline_get_predictors = caller.call_prepare_interp(subcsvv, module, weatherbundle, economicmodel, pvals[subbasename], farmer='noadapt')
                    ...
                    if check_doit(targetdir, subbasename + "-incadapt", suffix):
                        calculation, dependencies, baseline_get_predictors = caller.call_prepare_interp(subcsvv, module, weatherbundle, economicmodel, pvals[subbasename], farmer='incadapt')
