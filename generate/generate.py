...
for batchdir, pvals, clim_scenario, clim_model, weatherbundle, econ_scenario, econ_model, economicmodel in mode_iterators[config['mode']]():
    ...
    if config['mode'] not in ['writesplines', 'writepolys', 'diagnostic'] or config.get('do_historical', False):
        ...
        print "Historical"
        historybundle = weather.HistoricalWeatherBundle.make_historical(weatherbundle, None if config['mode'] == 'median' else pvals['histclim'].get_seed('yearorder'), pastyear_end=config.get('endhistorical', None))
        pvals.lock()
