...
with open('minT_10C_25C.csv', 'r') as fp:
...
lib.show_header("Calc. of minimum point temperature (%f reported)" % lib.excind(mintemps, 2009, 'min_T'))
...
lines = ["bl1(weather) = sum([%s]' * weather) / 100000" % ', '.join(["%.12g" % lib.excind(coeffs, 2009, coeff) for coeff in coefflist]),
         "bl(weather) = bl1(weather) - bl1(365 * [%s])" % (', '.join(["%d" % term for term in curve.get_terms(lib.excind(mintemps, 2009, 'min_T'))]))]
terms = []
...
lines = ["ef1(weather) = sum([%s]' * weather) / 100000" % ', '.join(["%.12g" % lib.excind(coeffs, futureyear, coeff) for coeff in coefflist]),
        "ef(weather) = ef1(weather) - ef1(365 * [%s])" % (', '.join(["%d" % term for term in curve.get_terms(lib.excind(mintemps, 2009, 'min_T'))]))]
...
if use_goodmoney:
    ...
    #          "deltacoeff = gdpgammas .* (%.12g - %.12g)" % (lib.excind(preds, futureyear - 1, 'loggdppc'), lib.excind(preds, 2009, 'loggdppc')),
    #          "ef1(weather) = sum(([%s] - deltacoeff)' * weather) / 100000" % ', '.join(["%.12g" % lib.excind(coeffs, futureyear, coeff) for coeff in coefflist]),
    #          "ef1(365 * [%s]) * 100000" % (', '.join(["%d" % term for term in curve.get_terms(lib.excind(mintemps, 2009, 'min_T'))]))]
    # lib.show_julia(lines)
