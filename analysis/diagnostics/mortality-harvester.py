...
for year in [2009, futureyear]:
    lib.show_header("Calc. of tas coefficient in %d (%f reported)" % (year, lib.excind(coeffs, year, 'tas')))
    if use_mle:
...
curve = PolynomialCurve([-np.inf, np.inf], [lib.excind(coeffs, 2009, coeff) for coeff in coefflist])
...
lines = ["bl1(weather) = sum([%s]' * weather) / 100000" % ', '.join(["%.12g" % lib.excind(coeffs, 2009, coeff) for coeff in coefflist]),
         "bl(weather) = bl1(365 * weather) - bl1(365 * [%s])" % (', '.join(["%.12g" % term for term in np.power(lib.excind(mintemps, 2009, 'analytic'), np.arange(1, 5))]))]
terms = []
for year in range(2001, 2011):
    terms.append("bl([%s])" % ', '.join(["%.12g" % lib.excind(calcs, year, 'var-%d' % ii) for ii in range(4)]))
...
lines.append("ef([%s]) - %f" % (', '.join(["%.12g" % lib.excind(calcs, futureyear, 'var-%d' % ii) for ii in range(4)]), lib.excind(calcs, futureyear, 'baseline')))
lib.show_julia(lines)
...
if use_goodmoney:
    ...
    lines = ["gdpgammas = [%s]" % ', '.join(["%.12g" % csvv['gamma'][ii] for ii in range(len(csvv['gamma'])) if csvv['covarnames'][ii] == 'loggdppc']),
             "transpose(gdpgammas) * [%s]" % ', '.join(["%.12g" % lib.excind(calcs, futureyear, 'var-%d' % ii) for ii in range(4)])]
    lib.show_julia(lines)
    ...
    #          "ef1([%s]) * 100000" % ', '.join(["%.12g" % lib.excind(calcs, futureyear, 'var-%d' % ii) for ii in range(4)])]
    # lib.show_julia(lines)
    ...
    lines = ["gdpgammas = [%s]" % ', '.join(["%.12g" % csvv['gamma'][ii] for ii in range(len(csvv['gamma'])) if csvv['covarnames'][ii] == 'loggdppc']),
             ...
             "ef1(weather) = sum(([%s] - deltacoeff)' * weather) / 100000" % ', '.join(["%.12g" % lib.excind(coeffs, futureyear, coeff) for coeff in coefflist]),
             "ef(weather) = ef1(365 * weather) - ef1(365 * [%s])" % (', '.join(["%.12g" % term for term in np.power(lib.excind(mintemps, 2009, 'analytic'), np.arange(1, 5))])),
             "ef([%s]) - %f" % (', '.join(["%.12g" % lib.excind(calcs, futureyear, 'var-%d' % ii) for ii in range(4)]), lib.excind(calcs, futureyear, 'baseline'))]
    lib.show_julia(lines)
