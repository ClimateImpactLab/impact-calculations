...
lines = []
lines.append("f_lt27(t1, t2, t3, tz) = " + ' + '.join(["%.12g * %s - %f * 27^%d * %f" % (lib.excind(calcs, 2000, 'lt27-' + coeff[ii]), var[ii], lib.get_gamma(csvv, coeff[ii], lt27_covars[ii]), ii+1, lib.excind(preds, 2000, 'colddd_10')) for ii in range(3)]))
lines.append("f_gt27(t1, t2, t3, tz) = " + ' + '.join(["%.12g * %s - %f * 27^%d * %f" % (lib.excind(calcs, 2000, 'gt27-' + coeff[ii]), var[ii], lib.get_gamma(csvv, coeff[ii], gt27_covars[ii]), ii+1, lib.excind(preds, 2000, 'hotdd_30')) for ii in range(3)]))
lines.append("f_both(t1, t2, t3, tz) = f_lt27(t1, t2, t3, tz) .* (t1 .< 27) + f_gt27(t1, t2, t3, tz) .* (t1 .>= 27)")
lines.append("f(t1, t2, t3, tz) = sum(f_both(t1, t2, t3, tz) - f_both(27, 27^2, 27^3, 0)) / 365")

line_base = '(' + ' + '.join(["f(weather_%d, weather_%d.^2, weather_%d.^3, 0)" % (year, year, year) for year in range(2001, 2011)]) + ') / 10'
...
lines = []    
lines.append("f_lt27(t1, t2, t3, tz) = " + ' + '.join(["%.12g * %s - %f * 27^min(2, %d) * %f" % (lib.excind(calcs, futureyear-1, 'lt27-' + coeff[ii]), var[ii], lib.get_gamma(csvv, coeff[ii], lt27_covars[ii]), ii+1, lib.excind(preds, futureyear-1, 'colddd_10')) for ii in range(3)]))
lines.append("f_gt27(t1, t2, t3, tz) = " + ' + '.join(["%.12g * %s - %f * 27^min(2, %d) * %f" % (lib.excind(calcs, futureyear-1, 'gt27-' + coeff[ii]), var[ii], lib.get_gamma(csvv, coeff[ii], gt27_covars[ii]), ii+1, lib.excind(preds, futureyear-1, 'hotdd_30')) for ii in range(3)]))
lines.append("f_both(t1, t2, t3, tz) = f_lt27(t1, t2, t3, tz) .* (t1 .< 27) + f_gt27(t1, t2, t3, tz) .* (t1 .>= 27)")
lines.append("f(t1, t2, t3, tz) = sum(f_both(t1, t2, t3, tz) - f_both(27, 27^2, 27^3, 0)) / 365")

line_result = "f(weather_%d, weather_%d.^2, weather_%d.^3, 0) - %.12g" % (futureyear, futureyear, futureyear, lib.excind(calcs, 2050, 'baseline'))
