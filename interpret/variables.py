...
def get_post_process(name, config, as_selfdoc):
    if 'final-t' in config:
        return wrap_as_selfdoc(as_selfdoc, lambda ds: post_process(ds, name, config), "Select time %d" % config['final-t'], docargs=[name])

    if 'within-season' in config:
        return wrap_as_selfdoc(as_selfdoc, lambda ds: post_process(ds, name, config), "Limit to within season", docargs=[name])
